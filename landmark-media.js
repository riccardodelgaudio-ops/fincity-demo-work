...
// Loads a representative Wikipedia / Wikimedia Commons image lazily for every trophy in every city.
(function(){
  const cachePrefix='fincity-landmark-media:v3:';
  ...
  async function wikipediaImage(t,city){
    ...
    const q=new URLSearchParams({action:'query',generator:'search',gsrsearch:search,gsrlimit:'5',prop:'pageimages|info',piprop:'thumbnail',pithumbsize:'1200',inprop:'url',format:'json',origin:'*'});
    ...
    return {url:p.thumbnail.source,source:'Wikipedia / Wikimedia',credit:p.title||t.name,license:'Bildlizenz siehe Quelle',page:p.fullurl||'https://de.wikipedia.org'};
  }

  async function commonsImage(t,city){
    const search=`${t.name} ${city} Switzerland`;
    const q=new URLSearchParams({action:'query',generator:'search',gsrsearch:search,gsrnamespace:'6',gsrlimit:'5',prop:'imageinfo',iiprop:'url|extmetadata',iiurlwidth:'1200',format:'json',origin:'*'});
    const r=await fetch('https://commons.wikimedia.org/w/api.php?'+q.toString());if(!r.ok)throw new Error('commons');
    const j=await r.json();const pages=Object.values(j.query?.pages||{}).sort((a,b)=>(a.index||99)-(b.index||99));
    for(const p of pages){const ii=p.imageinfo?.[0];if(!ii?.thumburl&&!ii?.url)continue;const ex=ii.extmetadata||{};const license=stripHtml(ex.LicenseShortName?.value||ex.UsageTerms?.value||'Wikimedia Commons');const artist=stripHtml(ex.Artist?.value||ex.Credit?.value||'Wikimedia Commons');const page='https://commons.wikimedia.org/wiki/'+encodeURIComponent(p.title.replace(/ /g,'_')).replace(/%3A/g,':');return {url:ii.thumburl||ii.url,source:'Wikimedia Commons',credit:artist||'Wikimedia Commons',license,page}}return null;
  }

  async function getMedia(t){
    const city=cityName();const cached=readCache(t,city);if(cached)return cached;
    let result=null;
    // Wikipedia page images are usually more semantically accurate for a named landmark.
    try{result=await wikipediaImage(t,city)}catch(e){}
    if(!result){try{result=await commonsImage(t,city)}catch(e){}}
  ...
  function enhanceTrophyList(){const rows=[...panel.querySelectorAll('[data-city-trophy]')];rows.forEach((row,i)=>{const id=Number(row.dataset.cityTrophy),t=trophies.find(x=>x.id===id);if(t)setTimeout(()=>addThumb(row,t),i*80)})}
