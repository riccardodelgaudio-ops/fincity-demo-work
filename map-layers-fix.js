...
(function(){
  ...
  function centerOfGeometry(g){
    const pts=geometryCoords(g);if(!pts.length)return null;
    let minX=Infinity,minY=Infinity,maxX=-Infinity,maxY=-Infinity;
    pts.forEach(c=>{if(!Array.isArray(c)||c.length<2)return;minX=Math.min(minX,c[0]);maxX=Math.max(maxX,c[0]);minY=Math.min(minY,c[1]);maxY=Math.max(maxY,c[1])});
  ...
  function snapBuildingsToFootprints(){
    ...
    const buildingLayer=map.getLayer('fincity-3d')?'fincity-3d':null;
    if(!buildingLayer)return;
    let changed=false;
    investment.forEach(p=>{
      const origin=map.project([p.lng,p.lat]);
      const pad=44;
      let rendered=[];
      ...
      rendered.forEach(f=>{
        const c=centerOfGeometry(f.geometry);if(!c)return;
        const px=map.project(c);const d=Math.hypot(px.x-origin.x,px.y-origin.y);
    ...
    if(changed)map.getSource(sourceId).setData(collection);
    snapped=true;
  }
  ...
  function installTouchSelection(){
    ...
    const pick=e=>{
      ...
      let hits=[];
      try{hits=map.queryRenderedFeatures([[p.x-18,p.y-18],[p.x+18,p.y+18]],{layers:allHit.filter(id=>map.getLayer(id))})||[]}catch(err){return}
      if(hits.length){if(e.preventDefault)e.preventDefault();openFeature(hits[0])}
    };
    map.on('click',pick);
    map.on('touchend',pick);
  }

  function addPointLayers(){
    if(!map||map.getSource(sourceId))return;
    removeHtmlMarkers();
    collection=buildFeatures();
  ...
  function setTypeVisibility(type,visible){
    [hitLayers[type],circleLayers[type],labelLayers[type]].forEach(id=>{if(map&&map.getLayer(id))map.setLayoutProperty(id,'visibility',visible?'visible':'none')});
  }

  window.applyFilter=function(type){
    document.querySelectorAll('.filter').forEach(b=>b.classList.toggle('active',b.dataset.filter===type));
    types.forEach(t=>setTypeVisibility(t,type==='all'||type===t));
  };

  function install(){if(!map)return;if(map.loaded())addPointLayers();else map.once('load',addPointLayers)}
  install();
})();
